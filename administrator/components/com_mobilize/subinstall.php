...
/**
 * Class for finalizing JSN Mobilize installation.
 *
 * @package  JSNSample
 * @since    1.0.0
 */
class Com_MobilizeInstallerScript extends JSNInstallerScript
{
	...
	public function preflight($mode, $parent)
	{
		...
		{
			//check if Mobilize is installed and version is smaller than 1.1.8 or not
			$mobilize = $this->getExtInfo();

			if (count($mobilize))
			{
				$extManifest        = json_decode($mobilize->manifest_cache);
				$installedVersion   = $extManifest->version;
				$this->preVersion 	= $installedVersion;
	...
	{
		$designsData = $this->getDesignData();

		if (count($designsData))
		{
			foreach ($designsData as $item)
			{
				$name      = $item->name;
				...
				if ($name == 'mobilize-style')
				{
					...
					if (count($value))
					{
						...
						foreach ($value as $k => $v)
						{
							...
							$contentDomain = $this->removeDomain($v);
							if ($contentDomain)
							{
								$pattern = 'components/com_mobilize/';
								$replaceWith = 'plugins/system/jsnmobilize/';
								$contentItem = $this->replaceOldPath($contentDomain, $pattern, $replaceWith);
							}

							$itemContainer[$k] = $contentItem;
						}

						if (count($itemContainer))
						{
							$itemContainer = json_encode($itemContainer);
							$this->updateDesignData($designID, $name, $itemContainer);
						}
					}
				}
				elseif ($name == 'mobilize-css')
				{
					$contentDomain   	= '';
					$contentItem   		= '';
					$contentDomain = $this->removeDomain($item->value);

					if ($contentDomain)
					{
						$pattern = '/administrator/components/com_mobilize/';
						$replaceWith = '/plugins/system/jsnmobilize/';
						$contentItem = $this->replaceOldPath($contentDomain, $pattern, $replaceWith);
					}
					...
					if ($contentItem)
					{
						$this->updateDesignData($designID, $name, $contentItem);
					}
				}
				else
				{
	...
	public function updateDesignData($designID, $name, $content)
	{
		if ($designID)
		{
			$db = JFactory::getDbo();
			...
			// Fields to update.
			$fields = array(
				$db->quoteName('value') . ' = ' . $db->quote($content)
			);

			// Conditions for which records should be updated.
			$conditions = array(
				$db->quoteName('design_id') . ' = ' . $designID,
				$db->quoteName('name') . ' = ' . $db->quote($name)
			);

			$query->update($db->quoteName('#__jsn_mobilize_design'))->set($fields)->where($conditions);
	...
	public function removeDomain($contents)
	{
		$pattern = '';
		if($_SERVER['SERVER_PORT'] !== '')
		{
			$pattern = 'http://' . $_SERVER['SERVER_NAME'] .':'.$_SERVER['SERVER_PORT']. JURI::root(true);
		}
		else
		{
			$pattern = 'http://' . $_SERVER['SERVER_NAME'] . JURI::root(true);
		}

		if ($pattern)
		{
			preg_match('#('.$pattern.')#', $contents, $results);
			$output = '';
			if (count($results))
			{
				$output = preg_replace('#('.$pattern.')#', '', $contents);
				return $output;
			}
		}
		return $contents;
	...
	public function replaceOldPath($contents, $pattern, $replaceWith = '')
	{
		if ($contents && $pattern) {
			preg_match('#('.$pattern.')#', $contents, $results);
			$output = '';
			if (count($results))
			{
				$output = preg_replace('#('.$pattern.')#', $replaceWith, $contents);
				return $output;
			}
		}
		return $contents;
...
//		}
		...
		$this->_cleanup(JPATH_ROOT . '/templates/jsn_mobilize/html');
...
<script type="text/javascript">
	if (window.parent)
		...
		else
		{
			if ($this->preVersion != '')
			{
				if (version_compare($this->preVersion, '1.1.8', '<'))
				{
					jexit('DONE');
				}
			}
	...
	{
		foreach (array('mod_virtuemart_category', 'com_virtuemart', 'mod_virtuemart_cart', 'mod_virtuemart_manufacturer', 'mod_virtuemart_product', 'mod_virtuemart_search') as $name)
		{
