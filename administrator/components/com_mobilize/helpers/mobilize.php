...
class JSNMobilizeHelper {
     ...
     * @return style code
     */
    public static function getStyleDesign($key,$valueStyle,$backGroundType){
    ...
    public static function getDependentExtensions()
    {
    	...
    	$indentifiedNames[JSN_MOBILIZE_IDENTIFIED_NAME] = JSN_MOBILIZE_VERSION;

    	$ext	= self::getJSNMobilizeTemplate();
    	if (count((array) $ext))
    	{
    		$manifest										= json_decode($ext->manifest_cache);
    		$indentifiedNames['ext_jsnmobilize_template']	= $manifest->version;
    	}
    	return $indentifiedNames;
    }

    public static function getJSNMobilizeTemplate()
    {
    	$db	= JFactory::getDbo();
    	$query	= $db->getQuery(true);
    	$query->select('*');
    	$query->from('#__extensions');
    	$query->where("type = 'template' AND element = 'jsn_mobilize'");
    	$db->setQuery($query);
