...
class JSNMobilizeModelProfile extends JModelAdmin {
    ...
    public function save($data) {
        ...
        if ($checkCreate) {
            if (empty($data['profile_id'])) {
                $db = $this->getDbo();
                $db->setQuery("UPDATE #__jsn_mobilize_profiles SET " . $db->quoteName('ordering') . " = ordering+1");
                $db->execute();
            }
            ...
            $query = "REPLACE INTO " . $db->quoteName('#__jsn_mobilize_config') . " (name, value) VALUES ('tmp_config', '')";
            $db->setQuery($query);
            if (!$db->execute()) {
    ...
    public function updateOsOrder($getOs){
        foreach ( $getOs as $os ) {
            ...
            $db = $this->getDbo();
            $db->setQuery("UPDATE #__jsn_mobilize_os SET " . $db->quoteName('os_order') . " = " .$order. " WHERE " . $db->quoteName('os_id') . " = ".$os->os_id);
            $db->execute();
