	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		foreach ($pks as $pk)
		{
			...
			if ($node->parent_id)
			{
				...
				$query = AriDBUtils::getQuery()->select('n.' . $k)->from($this->_db->quoteName($this->_tbl) . ' AS n')
			...
			$query = AriDBUtils::getQuery()->update($this->_db->quoteName($this->_tbl))->set('published = ' . (int) $state)
